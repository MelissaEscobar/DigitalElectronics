...
#Ahora se ordenará la lista según la cantidad de unos
    ...
    primImplicante = [] #lista en donde se guardaran los que son definitivamente primeros implicantes
    mix2Minterms = [] #lista en donde se guardaran los numeros binarios con el nuevo cambio
    mixMintermsAux = mix1Minterms

    while recorrer:
               
        r = mixAgain(mixMintermsAux)

        if r[2] == True: #si se pudo efectuar nuevos cambios en la lista de binarios, tal vez se puede
                        # recorrer la lista una vez mas
            primImplicante.append(r[0]) 
            mix2Minterms = r[1]
